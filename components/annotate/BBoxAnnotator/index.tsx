...
const BBoxAnnotator = React.forwardRef<any, Props>(
  ({ url, borderWidth = 2, inputMethod, labels, onChange }, ref) => {
    ...
    const [editingLabelForId, setEditingLabelForId] = useState<string | null>(
      null
    );
    ...
    const addEntry = (label: string) => {
      setEntries([
        ...entries,
        { ...rect, label, id: crypto.randomUUID(), showCloseButton: false },
      ]);
      setStatus("free");
      setPointer(null);
      setOffset(null);
    ...
    const submitEditLabel = (label: string) => {
      ...
      setEntries((prev) =>
        prev.map((e) => (e.id === editingLabelForId ? { ...e, label } : e))
      );
        ...
        setEntries((prev) =>
          prev.map((entry) => {
            ...
            const newLeft = Math.max(
              0,
              Math.min(
                (imageFrameStyle.width || 0) - entry.width - 1,
            ...
            const newTop = Math.max(
              0,
              Math.min(
                (imageFrameStyle.height || 0) - entry.height - 1,
          ...
          prev.map((entry) => {
            ...
            switch (resizing.handle) {
              case "e": {
                ...
                newWidth = Math.max(
                  minW,
                  Math.min(resizing.startRect.width + dx, maxWidth)
                );
                break;
              }
              case "s": {
                ...
                newHeight = Math.max(
                  minH,
                  Math.min(resizing.startRect.height + dy, maxHeight)
                );
                break;
              }
              case "se": {
              ...
              case "w": {
                ...
                newLeft = Math.max(
                  0,
                  Math.min(resizing.startRect.left + dx, maxLeft)
                );
                newWidth =
              ...
              case "n": {
                ...
                newTop = Math.max(
                  0,
                  Math.min(resizing.startRect.top + dy, maxTop)
                );
                newHeight =
            ...
            <div
              ...
              onMouseLeave={() =>
                setEntries((prevEntries) =>
                  prevEntries.map((e) =>
                    e.id === entry.id ? { ...e, showCloseButton: false } : e
                  )
                )
              }
              onMouseOver={() =>
                setEntries((prevEntries) =>
                  prevEntries.map((e) =>
                    e.id === entry.id ? { ...e, showCloseButton: true } : e
                  )
                )
              }
              style={{
